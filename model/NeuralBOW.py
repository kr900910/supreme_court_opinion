...
def embedding_layer(ids_, V, embed_dim, init_scale=0.001):
    ...
    W_embed_ = tf.get_variable(name="W_embed", shape=[V, embed_dim], initializer = 
                               tf.random_uniform_initializer(minval=(-1*init_scale), maxval=init_scale, seed=None, dtype=tf.float32), 
                               dtype=tf.float32)
...
def fully_connected_layers(h0_, hidden_dims, activation=tf.tanh,
    ...
    for i, hdim in enumerate(hidden_dims):
        ...
        if dropout_rate > 0:
            h_ = tf.layers.dropout(inputs=h_, rate=dropout_rate, training = is_training)  # replace with dropout applied to h_
...
def softmax_output_layer(h_, labels_, num_classes):
    ...
    with tf.variable_scope("Logits"):
        ...
        W_out_ = tf.get_variable(name="W_out", shape=[h_.shape[1],num_classes], 
                                 initializer=tf.random_normal_initializer(), dtype=tf.float32)
        b_out_ = tf.get_variable(name="b_out", shape=[num_classes], initializer=tf.zeros_initializer(), dtype=tf.float32)
...
def BOW_encoder(ids_, ns_, V, embed_dim, hidden_dims, dropout_rate=0,
    ...
    h_ = fully_connected_layers(h0_=h0_, hidden_dims=hidden_dims, dropout_rate=dropout_rate, is_training=is_training)
